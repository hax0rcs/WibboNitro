...
export class RoomSessionChatEvent extends RoomSessionEvent
{
    ...
    private _chatIcon: number;

    constructor(type: string, session: IRoomSession, objectId: number, message: string, chatIcon: number = 0, chatType: number, style: number = 0, links: string[] = null, extraParam: number = -1)
    {
        super(type, session);
