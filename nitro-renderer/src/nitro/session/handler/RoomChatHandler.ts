...
export class RoomChatHandler extends BaseHandler
{
    ...
    private onRoomUnitChatEvent(event: RoomUnitChatEvent): void
    {
        ...
        const chatEvent = new RoomSessionChatEvent(RoomSessionChatEvent.CHAT_EVENT, session, parser.roomIndex, parser.message, parser.chatIcon, chatType, parser.bubble);
    ...
    private onRespectReceivedEvent(event: RespectReceivedEvent): void
    {
        ...
        this.listener.events.dispatchEvent(new RoomSessionChatEvent(RoomSessionChatEvent.CHAT_EVENT, session, userData.roomIndex, '', 0, RoomSessionChatEvent.CHAT_TYPE_RESPECT, SystemChatStyleEnum.GENERIC));
    }
