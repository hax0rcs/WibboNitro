...
export class SessionDataManager extends NitroManager implements ISessionDataManager
{
    ...
    private onUserInfoEvent(event: UserInfoEvent): void
    {
        ...
        this._kissesLeft = userInfo.kissesRemaining;
    ...
    public giveKiss(userId: number): void
    {
        if((userId < 0) || (this._kissesLeft <= 0)) return;

        this.send(new KissUserComposer(userId));

        this._kissesLeft--;
    }

    public giveStar(userId: number): void
    {
        if(userId < 0) return;

        this.send(new GiveStarComposer(userId));
    ...
    public get kissesLeft(): number
    {
        return this._kissesLeft;
    }
