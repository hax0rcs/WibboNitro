import { AvatarExpressionEnum, GetTicker, RoomControllerLevel, RoomEngineObjectEvent, RoomObjectCategory, RoomRotatingEffect, RoomSessionChatEvent, RoomSettingsComposer, RoomShakingEffect, RoomZoomEvent, TextureUtils } from '@nitrots/nitro-renderer';
import { useEffect, useState } from 'react';
import { ChatMessageTypeEnum, CreateLinkEvent, GetConfiguration, GetRoomEngine, GetSessionDataManager, LocalizeText, SendMessageComposer } from '../../../api';
...
const useChatInputWidgetState = () =>
{
    ...
    const sendChat = (text: string, chatType: number, recipientName: string = '', styleId: number = 0, chatColour: string = '') =>
    {
        ...
        if(parts.length > 0)
        {
            ...
            switch(firstPart.toLowerCase())
            {
                ...
                case ':d':
                case ';d':
                    // if(GetClubMemberLevel() > HabboClubLevelEnum.NO_CLUB)
                    // {
                ...
                case ':floor':
                case ':bcfloor':
                    if(roomSession.controllerLevel >= RoomControllerLevel.GUEST) CreateLinkEvent('floor-editor/show');
    ...
    return { selectedUsername, floodBlocked, floodBlockedSeconds, chatVal, setChatVal, setIsTyping, setIsIdle, sendChat };
}
