import { ExtendedProfileChangedMessageEvent, RelationshipStatusInfoEvent, RelationshipStatusInfoMessageParser, RoomEngineObjectEvent, RoomObjectCategory, RoomObjectType, UserBannerComposer, UserBannerEvent, UserCurrentBadgesComposer, UserCurrentBadgesEvent, UserProfileEvent, UserProfileParser, UserRelationshipsComposer } from '@nitrots/nitro-renderer';
import { FC, useState } from 'react';
import { CreateLinkEvent, GetRoomSession, GetSessionDataManager, GetUserProfile, LocalizeFormattedNumber, LocalizeText, SendMessageComposer } from '../../api';
...
export const UserProfileView: FC<{}> = props =>
{
    ...
    useMessageEvent<UserProfileEvent>(UserProfileEvent, event =>
    {
        ...
        SendMessageComposer(new UserBannerComposer(parser.id));
    });
    ...
    return (
        <NitroCardView uniqueKey="nitro-user-profile" theme="primary-slim" className="user-profile">
            ...
            <NitroCardContentView overflow="hidden">
                ...
                <Flex alignItems="center" className="profile-badges-container px-2 py-1">
                <Grid columnCount={ 10 } fullHeight className="min-size bg-muted rounded px-2 py-1">
                            <BadgesContainerView fullWidth center badges={ userBadges } />
                        </Grid>
                        </Flex>
                <Flex alignItems="center" className="rooms-button-container px-2 py-1" gap={2}>
                    ...
                    <Flex alignItems="center" gap={ 1 }>
                        <i className="icon icon-level" />
                        <Text bold underline pointer>{ LocalizeText('extendedprofile.level') }</Text>
                        <Text bold>{ LocalizeFormattedNumber(userProfile.level) }</Text>
                    </Flex>
                    <div className="divider" />
                    <Flex alignItems="center" gap={ 1 }>
                        <i className="icon icon-star" />
                        <Text bold underline pointer>{ LocalizeText('extendedprofile.star') }</Text>
                        <Text bold>{ LocalizeFormattedNumber(userProfile.receivedDucket) }</Text>
                    </Flex>
                </Flex>
