...
import { CreateLinkEvent, DispatchUiEvent, GetConfiguration, GetSessionDataManager, MessengerIconState, OpenMessengerChat, RoomWidgetUpdateChatInputContentEvent, VisitDesktop } from '../../api';
import { Base, Flex, LayoutAvatarImageView, LayoutItemCountView, TransitionAnimation, TransitionAnimationTypes } from '../../common';
import { useAchievements, useChatInputWidget, useFriends, useInventoryUnseenTracker, useMessageEvent, useMessenger, useRoomEngineEvent, useSessionInfo } from '../../hooks';
...
export const ToolbarView: FC<{ isInRoom: boolean }> = props =>
{
    ...
    const { selectedUsername = '', floodBlocked = false, floodBlockedSeconds = 0, setIsTyping = null, setIsIdle = null, sendChat = null, chatVal = null, setChatVal = null } = useChatInputWidget();
            ...
            <Flex alignItems="center" justifyContent="between" gap={ 2 } className="nitro-toolbar py-1 px-3">
                <Flex gap={ 2 } alignItems="center">
                    <Flex alignItems="center" gap={ 3 }>
                        ...
                        { isMod &&
                            <><Base pointer className="navigation-item icon icon-modtools" onClick={event =>
                            {
                                CreateLinkEvent('mod-tools/show');
                                //CreateLinkEvent('custom-tools/show'); 
                            } } /><Base pointer className="navigation-item icon icon-staff-chat" onClick={event =>
                            {
                                DispatchUiEvent(new RoomWidgetUpdateChatInputContentEvent(RoomWidgetUpdateChatInputContentEvent.WHISPER, 'staffchat'));
                            } } /></>
                ...
                { /*<ChatAutoCompleteView chatValue={chatVal} setChatValue={setChatVal} /> */}
