...
export const WiredConditionActorHasRotiation: FC<{}> = props =>
{
    ...
    return <WiredConditionBaseView requiresFurni={ WiredFurniType.STUFF_SELECTION_OPTION_NONE } hasSpecialInput={ true } save={save}>
        ...
        <Flex justifyContent='center'><Text underline onClick={() => setMoreOptions(!moreOptions)}>{LocalizeText('wiredfurni.params.condition.rotation.more_options')}</Text></Flex>
        { moreOptions &&
        <Column gap={1}>
            <Flex><Text bold>{LocalizeText('wiredfurni.params.condition.rotation')}</Text></Flex>
