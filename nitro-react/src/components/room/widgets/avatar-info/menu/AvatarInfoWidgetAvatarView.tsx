...
import { CurrencyView } from '../../../../purse/views/CurrencyView';
...
export const AvatarInfoWidgetAvatarView: FC<AvatarInfoWidgetAvatarViewProps> = props =>
{
    ...
    const { userRespectRemaining = 0, userKissesRemaining = 0, kissUser = null, giveStar = null, respectUser = null } = useSessionInfo();
        ...
        if(name)
        {
            switch(name)
            {
                ...
                case 'back':
                    ...
                    break;
                case 'group_whisper':
                    roomSession.sendWhisperGroupMessage(avatarInfo.webID);
                    DispatchUiEvent(new RoomWidgetUpdateChatInputContentEvent(RoomWidgetUpdateChatInputContentEvent.WHISPER, 'group'));
                    break;
                case 'whisper':
                ...
                case 'givestar':
                {
                    giveStar(avatarInfo.webID);
                    break;
                }    
                case 'kiss': {
                    kissUser(avatarInfo.webID);
    
                    if((userKissesRemaining - 1) >= 1) hideMenu = false;
                    break;
                }
                case 'ignore':
        ...
        <ContextMenuView objectId={ avatarInfo.roomIndex } category={ RoomObjectCategory.UNIT } userType={ avatarInfo.userType } onClose={ onClose } collapsable={ true }>
            ...
            { (mode === MODE_NORMAL) &&
                <>
                    ...
                    <ContextMenuListItemView onClick={ event => processAction('group_whisper') }>
                        { LocalizeText('infostand.button.group_whisper') }
                    </ContextMenuListItemView>
                    ...
                    { (userRespectRemaining > 0) &&
                        ...
                        { (userKissesRemaining > 0) &&
                        <ContextMenuListItemView onClick={ event => processAction('kiss') }>
                            { LocalizeText('infostand.button.kiss.user', [ 'count' ], [ userKissesRemaining.toString() ]) }
                            </ContextMenuListItemView> 
                        }
                    <ContextMenuListItemView onClick={ event => processAction('givestar') }>
                            { LocalizeText('infostand.button.give.star') }
                            <CurrencyView type={ 105 } amount={ -1 } short={ true } />
                        </ContextMenuListItemView>
                    { !canRequestFriend(avatarInfo.webID) &&
