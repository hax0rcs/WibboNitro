import { RelationshipStatusInfoEvent, RelationshipStatusInfoMessageParser, RoomSessionFavoriteGroupUpdateEvent, RoomSessionUserBadgesEvent, RoomSessionUserFigureUpdateEvent, UserBannerComposer, UserBannerEvent, UserRelationshipsComposer } from '@nitrots/nitro-renderer';
import { Dispatch, FC, FocusEvent, KeyboardEvent, SetStateAction, useEffect, useState } from 'react';
import { FaPencilAlt, FaTimes } from 'react-icons/fa';
import { AvatarInfoUser, CloneObject, GetConfiguration, GetGroupInformation, GetSessionDataManager, GetUserProfile, LocalizeFormattedNumber, LocalizeText, SendMessageComposer } from '../../../../../api';
...
export const InfoStandWidgetUserView: FC<InfoStandWidgetUserViewProps> = props =>
{
    ...
    useRoomSessionManagerEvent<RoomSessionUserFigureUpdateEvent>(RoomSessionUserFigureUpdateEvent.USER_FIGURE, event =>
    {
        ...
        setAvatarInfo(prevValue =>
        {
            ...
            newValue.banner = event.banner;
            newValue.kissesReceibeved = event.kissesReceived;
            newValue.level = event.level;
    ...
    const bannerUrl = GetConfiguration('banner.url') as string;
    const backgroundImage = `url(${bannerUrl.replaceAll('%id%', avatarInfo.banner)})`;
    ...
    useEffect(() =>
    {
        ...
        SendMessageComposer(new UserBannerComposer(avatarInfo.webID));
    ...
    return (
        <Column className="rounded nitro-infostand">
            <Column overflow="visible" className="container-fluid content-area" gap={ 1 }>
                ...
                <Column gap={ 1 }>
                    <Flex gap={ 1 }>
                        <Column fullWidth alignItems="center" className="avatar-content" justifyContent="center">
                            <Flex className="body-image" style={{ backgroundImage }} onClick={ event => GetUserProfile(avatarInfo.webID) }>
                                <LayoutAvatarImageView figure={ avatarInfo.figure } direction={ 4 } />
                            </Flex>
                            { avatarInfo.groupId > 0 && <Flex center pointer={ (avatarInfo.groupId > 0) } className="badge-image group-image" onClick={ event => GetGroupInformation(avatarInfo.groupId) }>
                                { avatarInfo.groupId > 0 &&
                                    <LayoutBadgeImageView badgeCode={ avatarInfo.groupBadgeId } isGroup={ true } showInfo={ true } customTitle={ avatarInfo.groupName } /> }
                            </Flex> }
                            { avatarInfo.groupId <= 0 &&
                            <Flex center pointer={ false } className="badge-image group-image">
                            { avatarInfo.groupId > 0 &&
                                <LayoutBadgeImageView badgeCode={ '' } isGroup={ true } showInfo={ false } customTitle={ 'none' } /> }
                                </Flex>
                            }
                        </Column>
                        <Column grow alignItems="center" gap={ 0 }>
                            <Flex gap={ 1 }>
                                <Flex center className="badge-image">
                                    { avatarInfo.badges[0] && <LayoutBadgeImageView noCache={ true } badgeCode={ avatarInfo.badges[0] } showInfo={ true } /> }
                                </Flex>
                                <Flex center className="badge-image">
                                    { avatarInfo.badges[1] && <LayoutBadgeImageView noCache={ true } badgeCode={ avatarInfo.badges[1] } showInfo={ true } /> }
                                </Flex>
                            </Flex>
                            <Flex center gap={ 1 }>
                                <Flex center className="badge-image">
                                    { avatarInfo.badges[2] && <LayoutBadgeImageView noCache={ true } badgeCode={ avatarInfo.badges[2] } showInfo={ true } /> }
                                </Flex>
                                <Flex center className="badge-image">
                                    { avatarInfo.badges[3] && <LayoutBadgeImageView noCache={ true } badgeCode={ avatarInfo.badges[3] } showInfo={ true } /> }
                                </Flex>
                            </Flex>
                            <Flex center gap={ 1 }>
                                <Flex center className="badge-image">
                                    { avatarInfo.badges[4] && <LayoutBadgeImageView noCache={ true } badgeCode={ avatarInfo.badges[4] } showInfo={ true } /> }
                                </Flex>
                                <Flex center className="badge-image">
                                    { avatarInfo.badges[5] && <LayoutBadgeImageView noCache={ true } badgeCode={ avatarInfo.badges[5] } showInfo={ true } /> }
                                </Flex>
                            </Flex>
                            <Flex center gap={ 1 }>
                                <Flex center className="badge-image">
                                    { avatarInfo.badges[6] && <LayoutBadgeImageView noCache={ true } badgeCode={ avatarInfo.badges[6] } showInfo={ true } /> }
                                </Flex>
                                <Flex center className="badge-image">
                                    { avatarInfo.badges[7] && <LayoutBadgeImageView noCache={ true } badgeCode={ avatarInfo.badges[7] } showInfo={ true } /> }
                                </Flex>
                            </Flex>
                            <Flex center gap={ 1 }>
                                <Flex center className="badge-image">
                                    { avatarInfo.badges[8] && <LayoutBadgeImageView noCache={ true } badgeCode={ avatarInfo.badges[8] } showInfo={ true } /> }
                                </Flex>
                                <Flex center className="badge-image">
                                    { avatarInfo.badges[9] && <LayoutBadgeImageView noCache={ true } badgeCode={ avatarInfo.badges[9] } showInfo={ true } /> }
                                </Flex>
                            </Flex>
                        </Column>
                    </Flex>
                ...
                <Column gap={ 1 }>
                    <Flex alignItems="center" className="px-2 py-1 rounded bg-light-dark">
                ...
                <Column gap={ 1 }>
                    ...
                    <Text variant='white' small> { LocalizeFormattedNumber(avatarInfo.achievementScore) }
                    </Text>
                ...
                <Text variant="white" small wrap>
                    <Flex>
                    <Text grow variant='white' underline>{ LocalizeText('infostand.text.level') } </Text>
                    </Flex>
                    <Flex>
                    <Text variant='white' small> { LocalizeFormattedNumber(avatarInfo.level) } </Text>
                    </Flex>
                </Text>
                </Flex>
                <Flex gap={1}>
                <Text variant="white" small wrap>
                    <Flex>
                    <Text grow variant='white' underline>{ LocalizeText('infostand.text.kisses_received') } </Text>
                    </Flex>
                    <Flex>
                    <Text variant='white' small> { LocalizeFormattedNumber(avatarInfo.kissesReceibeved) } </Text>
                    </Flex>
                </Text>
                </Flex>
                </Column>
