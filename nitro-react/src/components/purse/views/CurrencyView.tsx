...
export const CurrencyView: FC<CurrencyViewProps> = props =>
{
    ...
    {
        return (
            <Flex justifyContent="end" alignItems="center" pointer gap={ 1 } className="nitro-purse-button rounded" onClick={ () => HabboWebTools.openPage('/shop') }>
                <LayoutCurrencyIcon type={ type } />
                <Text truncate textEnd className={`colour-${type}`} grow>{ short ? LocalizeShortNumber(amount) : LocalizeFormattedNumber(amount) }</Text>
            </Flex>);
    }, [ amount, short, type ]);
