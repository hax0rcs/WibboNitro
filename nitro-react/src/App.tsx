    ...
    const handler = useCallback(async (event: NitroEvent) =>
    {
        switch (event.type)
        {
            ...
            case ConfigurationEvent.FAILED:
                setIsError(true);
                setMessage('Erro na Configuração');
                return;
            case Nitro.WEBGL_UNAVAILABLE:
                setIsError(true);
                setMessage('WebGL é requerido');
                return;
            ...
            case NitroCommunicationDemoEvent.CONNECTION_ERROR:
                setIsError(true);
                setMessage('Houve um erro entre a conexão do servidor.');
                return;
            case NitroCommunicationDemoEvent.CONNECTION_CLOSED:
                if(GetNitroInstance().roomEngine) GetNitroInstance().roomEngine.dispose();
                setIsError(true);
                setMessage('A conexão foi encerrada!');
            ...
            case NitroLocalizationEvent.LOADED: {
                ...
                {
                    setIsError(true);
                    setMessage('Erro no processamento do AssetManager');
                }
                return;
            }
        }
